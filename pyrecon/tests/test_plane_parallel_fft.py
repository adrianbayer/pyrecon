...
def test_ref(data_fn, randoms_fn, data_fn_rec=None, randoms_fn_rec=None):
    ...
    recon = PlaneParallelFFTReconstruction(f=0.8, bias=2., los='x', boxcenter=boxcenter, boxsize=boxsize, nmesh=128, dtype='f8')
    recon.assign_data(data['Position'], data['Weight'])
    recon.assign_randoms(randoms['Position'], randoms['Weight'])
    recon.set_density_contrast()
    recon.mesh_delta += 10.
...
if __name__ == '__main__':
    ...
    test_dtype()
    test_wrap()
    test_mpi(PlaneParallelFFTReconstruction, with_local_los=False)
