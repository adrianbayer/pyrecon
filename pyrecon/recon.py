...
class BaseReconstruction(BaseClass):
    ...
    def _mesh_positions(self):
        partition = self.pm.plans['partitionT']
        coord = np.moveaxis(np.indices(partition.local_i_shape, dtype='i4'), 0, -1)
    ...
    def set_f(self, f):
        ...
        if callable(f):
            self.f_callable = f
            if not self._f_z:
                raise ValueError('{} does not support redshift-evolving f')
            mesh = self.pm.create('real', value=0.)
            for rslab, slab in zip(mesh.slabs.x, mesh.slabs):
                rslab = self._transform_rslab(rslab)
                slab[...] = f(sum(rr**2 for rr in rslab)**0.5)
            self.f = mesh
        else:
            self.f = f
    ...
    def set_bias(self, bias):
        if callable(bias):
            ...
            for rslab, slab in zip(mesh.slabs.x, mesh.slabs):
                rslab = self._transform_rslab(rslab)
                slab[...] = bias(sum(rr**2 for rr in rslab)**0.5)
            self.bias = mesh
        else:
            self.bias = bias
    ...
    def _set_optimal_weights(self, P0=None, nbar=None, alpha=1., factor=1):
        ...
        if nbar is not None:
            for rslab, slab in zip(self.mesh_delta.slabs.x, self.mesh_delta.slabs):
                rslab = self._transform_rslab(rslab)
                slab[...] *= func(nbar(sum(rr**2 for rr in rslab)**0.5))
        else:  # density estimated as alpha * mesh_randoms
            for rslab, slab in zip(self.mesh_randoms.slabs, self.mesh_delta.slabs):
                slab[...] *= func(alpha * rslab)
    ...
    def set_density_contrast(self, ran_min=0.01, smoothing_radius=15., check=False, kw_weights=None):
        ...
        kw_weights : dict, default=None
            Optionally, for optimal weights, dictionary with keys 'P0' (fiducial power spectrum value),
